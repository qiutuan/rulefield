...
| `rulefield.template.loader` | 分组加载（`templates/<group>/<name>.yml`）、extends 继承、field_sets 复用、版本/启用开关、热加载（mtime 轮询，watchdog 可选）、失败隔离（坏模板保留上一可用版本） | `TemplateLibrary` | M2 |
...
| `rulefield.executor.workers` | 线程池并发；**按输入序输出**；单条/单文件失败隔离 | `BatchExecutor` | M3 |
| `rulefield.executor.checkpoint` | 断点续跑：已处理文件清单；resume 跳过已完成文件 | `Checkpoint` | 冗余/容错 |
...
| # | 决策 | 理由 |
| --- | --- | --- |
| D1 | 正则用 `regex` 库并带 `timeout`（默认 0.1s/次） | 标准 `re` 无超时；灾难性回溯会拖垮整批任务；超时样本按失败落盘不中断 |
...
| D7 | 热加载默认 mtime 轮询（间隔可配，默认 3s），watchdog 为可选依赖 | 零额外依赖；模板库整体原子换引 |
| D8 | 模板加载失败隔离：坏模板报 文件:行:列 并拒绝加载，**保留上一可用版本**，其余模板不受影响 | 满足验收第 4 条 |
| D9 | 去重集合在内存；超大规模（>100 万 key）给告警 | 目标内存 <100MB；不做数据库（硬约束） |
| D10 | YAML 用 `PyYAML`(CLoader) 而非 ruamel.yaml | 模板是机器读写的配置，无 round-trip 保留注释需求；CLoader 加载快数倍；错误定位自研校验器已覆盖 |
| D11 | 每字段三态：captured / missing / failed，逐字段可解释 | 支撑置信标记与工作台 diff |
| D12 | 记录状态机：success / partial / failed + 置信 complete / partial / low | 验收第 1 条“命中率”有明确定义口径 |
...
class FieldResult:
    ...
    status: str              # captured | missing | failed
    error: str | None        # 失败原因（regex 未命中/校验失败/类型转换失败…）
    pipeline_steps: list     # (op, 输入, 输出) 供工作台展示

@dataclass
class ExtractRecord:
    fields: dict[str, FieldResult]
    status: str              # success | partial | failed
    confidence: str          # complete | partial | low
    template: str
    template_version: str
    strategy: str            # 生效策略 id
...
- `success`：策略命中且所有声明字段 captured 且校验通过。
- `partial`：策略命中，但存在可选字段 missing 或非 required 校验失败。
- `failed`：无策略命中，或 required 字段 missing / 校验失败。

## 6. 错误处理与可观测性（三层隔离）

| 层 | 失败形态 | 处理 |
| --- | --- | --- |
| 单条文本 | 解码失败 / regex 超时 / 校验失败 | 记为 failed 记录 → failures.jsonl；不中断 |
...
- 开发：`pytest`、`pytest-cov`、`ruff`、`black`、`watchdog`(可选热加载)
